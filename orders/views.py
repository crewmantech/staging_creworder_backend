...
class OrderStatusWorkflowViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        # 🔒 DO NOT touch order_status or shipment_vendor
        # They are unique identity fields
        ...
        if allow_status_data is not None:
            ...
            workflow.allow_status.set(
                AllowStatus.objects.filter(id__in=allow_status_ids)
            )
...
class CheckPhoneDuplicateAPIView(APIView):
    ...
    def get(self, request):
        ...
        # 2️⃣ Lead / Follow-Up
        # ----------------------------
        if not resolved_phone and reference_id:
            try:
                ...
                if ref and ref.get("phone_number"):
                    resolved_phone = normalize_phone(ref["phone_number"])
                    source = ref["type"]
            except ValidationError:
                pass  # continue fallback

        # ----------------------------
        # 3️⃣ Appointment (IMPORTANT)
        # ----------------------------
        if not resolved_phone and reference_id:
            ...
            if appointment_phone:
                resolved_phone = normalize_phone(appointment_phone)
                source = "appointment"
        ...
        if not resolved_phone and reference_id:
            try:
                phone = get_phone_number_by_call_id(
                    user=request.user,
